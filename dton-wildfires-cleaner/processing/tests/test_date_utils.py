...
from processing.date_utils import get_exact_date, get_visualization_dates, start_of_day, end_of_day
...
@pytest.mark.parametrize(
    ...
    [
        (
            BBox((153.0079394411468, -25.12443961167254, 153.11725920035838, -25.27724579749874), crs=CRS.WGS84),
            ...
            0.5,
            0.4,
            ["get_visualization_dates_wfs_before.json", "get_visualization_dates_wfs_after.json"],
            [
                {
...
def test_get_visualization_dates(
    ...
    for filename in wfs_filenames:
        responses.add(
            responses.GET, re.compile(r"^.*sentinel-hub.com/ogc/wfs/.*$"), json=get_response_data(filename), status=200
        )
