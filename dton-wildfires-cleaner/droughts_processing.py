...
from processing.fis_utils import get_best_fis_dates
...
def process_drought_event(event, settings, visualization_settings, config, verbose=False):
    ...
    fis_request = FisRequest(
        ...
        layer=fis_layer,
        geometry_list=[bbox],
        time=(from_time_ndvi, to_time_ndvi),
        resolution=fis_resolution,
        custom_url_params={CustomUrlParam.EVALSCRIPT: ndvi_fis_evalscript},
        maxcc=FIS_MAXCC,
        bins=1,
        config=config,
    )
    ...
    try:
        refined_bbox = get_bbox(
            features, bbox, LIMIT_INCLUDED, LIMIT_SCORE, limit_size=LIMIT_SIZE, sizes_as_weights=True, verbose=verbose
        )
    ...
    visualization_dates = construct_visualization_dates_droughts(
        best_before_drought_date.strftime("%Y-%m-%d"), best_drought_date.strftime("%Y-%m-%d")
    )
