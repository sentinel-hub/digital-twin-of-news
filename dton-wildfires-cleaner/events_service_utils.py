...
class EventsService:
    ...
    def set_events_confirmed_status(self, events, status):
        for event in events:
            res = self.session.patch(f'{self.base_url}/v1/events/{event["id"]}', data=json.dumps({"confirmed": status}))
    ...
    def update_existing_event(self, eventId, lat=None, lng=None, zoom=None, confirmed=None, visualization_dates=None):
        payload = self.construct_payload(
            lat=lat, lng=lng, zoom=zoom, confirmed=confirmed, visualization_dates=visualization_dates
        )
        if self.dev_mode:
            pprint(payload)
            return
    ...
    def construct_payload(self, lat=None, lng=None, zoom=None, confirmed=None, visualization_dates=None):
        payload = {}
        if lat is not None:
            payload["lat"] = lat
        if lng is not None:
