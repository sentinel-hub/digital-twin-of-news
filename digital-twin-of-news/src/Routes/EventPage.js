...
import { LayersFactory, BBox, CRS_EPSG4326, WmsLayer } from '@sentinel-hub/sentinelhub-js';
...
import { LAYERS_PNG_ONLY, EFFIS_VIIRS_FIRES, only_show_layers_with_specific_dates } from '../const';
...
function EventPage(props) {
  ...
  const [visualizationDates, setVisualizationDates] = useState();
  ...
  useEffect(() => {
    ...
    if (event && selectedLayerId) {
      getVisualizationDates(
  ...
  useEffect(() => {
    ...
    if (event && event.type) {
      ...
      const hasEffisLayer = !!event.visualizationDates[EFFIS_VIIRS_FIRES.id];
      ...
      fetchLayers(apiKey, event.type, event.visualizationDates, hasEffisLayer, datasetsToDisplay);
    }
  }, [event, error, history]);

  const fetchLayers = async (apiKey, eventType, visualizationDates, hasEffisLayer, datasetsToDisplay) => {
    ...
    if (hasEffisLayer) {
      const effisLayer = new WmsLayer({
        baseUrl: EFFIS_VIIRS_FIRES.baseUrl,
        layerId: EFFIS_VIIRS_FIRES.layerId,
      });
      effisLayer.dataset = { id: EFFIS_VIIRS_FIRES.id };
      effisLayer.title = EFFIS_VIIRS_FIRES.title;
      layers.push(effisLayer);
    }
    ...
    setSelectedLayerId(sortedLayers[0].layerId);
  };
  ...
  async function getAndSetPrevNextDate(currentDate, position, direction) {
    if (isWMSLayer(selectedLayerId)) {
      ...
      setVisualizationDates(newVisDates);
    } else {
      const layer = await LayersFactory.makeLayer(
