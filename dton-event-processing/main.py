...
from queries import (
    ...
    droughts_query,
    air_pollution_query,
    air_pollution_query_wildfires,
    air_pollution_query_sandstorms,
)
from concept_thresholds import (
    volcano_thresholds,
    wildfire_thresholds,
    droughts_thresholds,
...
event_registry = EventRegistry(EVENT_REGISTRY_API_KEY, use_cache=DEV_MODE or USE_EVENT_REGISTRY_CACHE)
    ...
    for event_type in events_types_to_run:
        if event_type == EventType.WILDFIRE:
            ...
            events_media_intelligence = event_registry.get_events_from_media_intelligence(
                wildfires_query(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
                include_event_concepts=True,
            )
            ...
            config.instance_id = INSTANCE_ID_WILDFIRES
            events_media_intelligence = event_registry.get_events_from_media_intelligence(
                volcanoes_query(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
                include_event_concepts=True,
            )
            print(f"Original n of events: {len(events_media_intelligence)}")
            events_media_intelligence = filter_events_by_concept_thresholds(
            ...
            events_media_intelligence = event_registry.get_events_from_media_intelligence(
                droughts_query(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
                include_event_concepts=True,
            )
            print(f"Original n of events: {len(events_media_intelligence)}")
            events_media_intelligence = filter_events_by_concept_thresholds(
            ...
            air_pollution_events = event_registry.get_events_from_media_intelligence(
                air_pollution_query(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
                include_event_concepts=True,
            )
            air_pollution_events = filter_events_by_concept_thresholds(air_pollution_events, air_pollution_thresholds)
            print(f"{len(air_pollution_events)} standard air pollution events.")

            air_pollution_events_wildfires = event_registry.get_events_from_media_intelligence(
                air_pollution_query_wildfires(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
                include_event_concepts=True,
            )
            air_pollution_events_wildfires = filter_events_by_concept_thresholds(
                air_pollution_events_wildfires, air_pollution_thresholds_widfires
            )
            print(f"{len(air_pollution_events_wildfires)} air pollution events connected to wildfires.")

            air_pollution_events_sandstorms = event_registry.get_events_from_media_intelligence(
                air_pollution_query_sandstorms(MEDIA_INTELLIGENCE_DATE_START, MEDIA_INTELLIGENCE_DATE_END),
            )
            print(f"{len(air_pollution_events_sandstorms)} air pollution events connected to sandstorms.")

            events_media_intelligence = [
                *air_pollution_events,
                *air_pollution_events_wildfires,
                *air_pollution_events_sandstorms,
            ]
            print(f"Original n of events: {len(events_media_intelligence)}")
            events_monitoring = []
        else:
            print(f"Processing for {event_type} isn't implemented yet.")
            continue
